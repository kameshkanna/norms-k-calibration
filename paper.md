...
Despite its promise, activation steering faces a fundamental calibration problem. The intervention magnitude $K$ has no principled default: values that are too small produce no behavioral change; values that are too large collapse the model's outputs to a single degenerate mode. Existing practice addresses this with grid search over $K$ for each model, layer, and behavior — a process that does not scale to production deployments and transfers poorly across architectures. Turner et al. (2023) use a fixed scalar $K$; Rimsky et al. (2024) scale by the contrastive mean vector norm; Zou et al. (2023) hand-tune per experiment. To our knowledge, no prior work proposes a closed-form, architecture-agnostic formula for $K$ derived from activation norms or weight geometry.
...
2. **Spectral validation** (Section 5.2, Figure 1): per-layer $K$ values correlate with MLP $W_\text{up}$ spectral norms at $r = 0.71$–$0.77$ in pre-norm architectures ($p < 10^{-5}$), and at $r = 0.960$ (Spearman $\rho = 0.9992$) with accumulated post-norm scales in Gemma 2, unifying the formula across normalization regimes.
...
| E6 | Raw activation control | Alignment is specific to contrastive behavioral variance | Behavioral advantage over raw PC1 |
| E7 | Gemma 2 post-norm proxy | $K_\ell$ tracks cumulative $\|\gamma^\text{post}\|$ in dual-norm architectures | Pearson $r$, Spearman $\rho$ vs. multiple proxies |
...
**Table 1.** Pearson $r$ between per-layer $K_\ell$ and two spectral proxies: $\sigma_1(W_\text{up})$ (MLP up-projection spectral norm) and the cumulative post-norm scale $\sum_{\ell'<\ell}\|\gamma^\text{post}_{\ell'}\|$ (Gemma 2 only).

| Model | $r$ (K vs. $\sigma_1(W_\text{up})$) | $p$-value | $r$ (K vs. cumul. $\|\gamma^\text{post}\|$) |
|-------|--------------------------------------|-----------|----------------------------------------------|
| Llama 3.1 8B | 0.769 | $2.7 \times 10^{-7}$ | — |
| Qwen 2.5 7B | 0.711 | $2.2 \times 10^{-5}$ | — |
| Mistral 7B | 0.715 | $4.2 \times 10^{-6}$ | — |
| Gemma 2 9B | 0.421 | $5.4 \times 10^{-3}$ | **0.960** ($\rho = 0.9992$, $p < 10^{-23}$) |

For three pre-norm architectures, $r > 0.71$ at $p < 10^{-5}$, confirming the formula implicitly recovers MLP spectral scale. For Gemma 2, the $\sigma_1(W_\text{up})$ correlation is weak (Spearman $\rho = 0.12$, $p = 0.44$ — not significant), but replacing the spectral norm with the cumulative post-norm scale recovers $r = 0.960$ (Spearman $\rho = 0.9992$). This confirms the theoretical prediction in Section 6.1: the K formula universally tracks residual stream increment magnitudes — from weight spectra in pre-norm architectures, and from normalization scale parameters in dual-norm architectures. $K$ also correlates negatively with $W_\text{down}$ spectral norms ($r \approx -0.45$ to $-0.72$ for pre-norm models), consistent with the circuit interpretation that up-projection amplifies and down-projection compresses. Figure 1 visualizes these per-layer relationships.
...
Three of four models show a positive behavioral advantage over raw PC1: Mistral +7.43×, Llama +1.26×, Qwen +0.77×. These advantages differ materially in credibility. Mistral's +7.43× is robust — a 2.6× multiplicative lift over raw PC1 on 45 pairs. Llama's +1.26× is moderate and plausibly real. Qwen's +0.77× is marginal given the 45-pair dataset and should be treated as suggestive rather than confirmatory; a dataset of 100+ pairs per behavior would be needed to reduce this variance. Gemma 2 shows no behavioral advantage (−0.63×), consistent with its dual-norm architecture elevating all structured residual-stream directions equally (Section 6.1). The contrastive mean direction tracks closely with behavioral PC1 for most models, as expected — both derive from the same contrastive activations.
...
### 6.1 Gemma 2 Resolves: The K Formula Is a Universal Spectral Bridge

Three Gemma 2 anomalies appeared to resist a unified explanation: weak K–spectral correlation ($r = 0.42$), behavioral alignment without specificity (Table 3, −0.63×), and behaviorally elevated raw PC1 alignment (5.61×). Experiment 09 confirms they share a single architectural cause — and that the K formula is a *stronger* predictor for Gemma 2 than for any pre-norm model when the correct proxy is used.

**Why the K–spectral correlation is weak.** In pre-norm transformers:

$$x_{\ell+1}^{\text{(pre-norm)}} = x_\ell + F\!\left(\text{RMSNorm}_\text{pre}(x_\ell)\right)$$

each sublayer increment has magnitude $\sim \sigma_1(W_\ell) \cdot \sqrt{d}$, so the residual norm $\bar\mu_\ell$ integrates spectral scales and $K_\ell$ serves as a spectral proxy. In Gemma 2:

$$x_{\ell+1}^{\text{(Gemma 2)}} = x_\ell + \text{RMSNorm}_\text{post}\!\left(F\!\left(\text{RMSNorm}_\text{pre}(x_\ell)\right)\right)$$

the post-sublayer RMSNorm clips every increment to $\|\gamma^\text{post}_\ell\|_\text{eff} \cdot \sqrt{d}$ independently of $\sigma_1(W_\ell)$. The residual norm $\bar\mu_\ell$ integrates post-norm scales, not weight spectral scales. So $K_\ell$ should correlate strongly with the cumulative $\|\gamma^\text{post}_\ell\|_\text{eff}$ — not with $\sigma_1(W_\text{up})$.

**Experiment 09 confirms this exactly.** For Gemma 2 ($n = 42$ layers):

- $r(K_\ell,\, \sigma_1(W_\text{up})) = 0.421$ — Spearman $\rho = 0.123$, $p = 0.44$ (not significant)
- $r(K_\ell,\, \|\gamma^\text{post}_\text{MLP}\|) = 0.940$ — $p < 10^{-19}$
- $r(K_\ell,\, \text{cumul.}\,\|\gamma^\text{post}\|) = \mathbf{0.960}$ — Spearman $\rho = \mathbf{0.9992}$, $p < 10^{-23}$

The cumulative combined scale achieves near-perfect monotonic correlation. The K formula works for Gemma 2 — it simply tracks a different spectral proxy: accumulated normalization scale rather than weight spectral norms. The formula $K_\ell = \bar\mu_\ell/\sqrt{d}$ is architecture-agnostic by design; the physics that determines which proxy dominates is architectural.

**Why behavioral specificity disappears for Gemma 2.** With all sublayer increments clipped to equal magnitude, the dominant residual stream direction accumulates from the mean of all sublayer contributions. This mean direction aligns strongly with $W_\text{down}$ SVs independent of behavioral content — so raw PC1 (the dominant generic direction) achieves high alignment (5.61×). The behavioral signal is present (3.04×) but indistinguishable from the elevated generic baseline. This is not a failure of the spectral constraint; it is the constraint operating on an architecture where generic residual-stream directions are already structurally biased.

The three pre-norm architectures provide behavioral *specificity* (directional bias above the generic baseline) because their residual streams are behaviorally inflected — weight spectra selectively amplify contrastive variance. Gemma 2 provides behavioral *alignment* (3.04× above random) without specificity, because post-norm normalization decouples spectral amplification from behavioral content. Both regimes are captured by the K formula; they differ in what the formula's proxy measures.

### 6.2 The K Formula as a Universal Spectral Bridge

The results across all five experiments tell a unified story. In pre-norm transformers, $\bar\mu_\ell$ is a running integral of MLP spectral scales (Section 5.2, Figure 1), and $K_\ell$ tracks $\sigma_1(W_\text{up})$ at $r = 0.71$–$0.77$. In Gemma 2's dual-norm architecture, $\bar\mu_\ell$ is a running integral of post-norm scales, and $K_\ell$ tracks $\|\gamma^\text{post}_\ell\|$ at $r = 0.960$ (Spearman $\rho = 0.9992$, Section 6.1). In both regimes, $K_\ell = \bar\mu_\ell/\sqrt{d}$ measures the effective per-layer perturbation scale in the residual stream. Behavioral PCA directions preferentially occupy the dominant subspaces of the MLP weight matrices (Section 5.3), and the specificity of this alignment depends on whether the residual stream geometry is behaviorally inflected or architecturally flattened (Section 5.4). We call this the *spectral bridge*: activation norms serve as the universal mediating quantity linking weight-space geometry (spectral or normalization-parameterized) to activation-space intervention scale.

This picture connects to the Platonic Representation Hypothesis (Huh et al., 2024) at a local level. Behaviorally relevant activation subspaces are constrained by the spectral structure that governs information flow — and the K formula recovers that structure across fundamentally different normalization regimes without any architectural specialization.

---

## 7. Limitations

*(i)* **Efficacy metric.** We evaluate on activation-space cosine accuracy — whether steered activations move toward the target behavioral direction. This is a necessary but not sufficient condition for downstream behavioral change. Human evaluations or classifier-based behavioral metrics would provide a more direct measure. *(ii)* **Scale.** We test only 7–9B parameter models; whether the K formula's spectral interpretation holds at 70B+ is unknown. *(iii)* **Calibration robustness.** The calibration set of 50 prompts is small; sensitivity of $\bar\mu_\ell$ to prompt distribution and domain shift has not been characterized. *(iv)* **Permutation scope.** Permutation invariance experiments use only random MLP neuron permutations; structured permutations (attention-head rearrangements, cross-layer permutation groups) may exhibit different behavior. *(v)* **Isotropy assumption.** The K formula derivation assumes $\mathbb{E}[(\mathbf{v}^\top \hat{\mathbf{x}})^2] = 1/d$, which holds exactly for uniform-sphere inputs but not for transformer residual streams (which are highly anisotropic, as Section 5.4 confirms). The formula is therefore a spectral prior, not an exact derivation. *(vi)* **Behavioral specificity scope.** Gemma 2 shows no behavioral advantage over raw PC1 in the specificity control. Section 6.1 and Experiment 09 provide a mechanistic account (post-norm elevates all directional alignments), but the causal path from architectural choice to behavioral non-specificity is established empirically rather than derived.
